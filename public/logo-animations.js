// Logo Animation Script - Apply beautiful animations to all logos
(function() {
  'use strict';
  ...
  ready(function() {
    // Apply animations after a short delay to ensure React has rendered
    setTimeout(applyLogoAnimations, 500);
    
    // Re-apply animations when new content is loaded (for React SPA)
    const observer = new MutationObserver(function(mutations) {
      mutations.forEach(function(mutation) {
        if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
          setTimeout(applyLogoAnimations, 100);
        }
      });
    });
